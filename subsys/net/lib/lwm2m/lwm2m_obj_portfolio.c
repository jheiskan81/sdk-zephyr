...
/* Support only 1 even this multi instance object */
...
static struct lwm2m_engine_obj_field fields[] = {
	OBJ_FIELD_DATA(PORTFOLIO_IDENTITY_ID, RW, STRING), /* Mandaory Multi Instance */
	OBJ_FIELD_EXECUTE(PORTFOLIO_GET_AUTH_DATA_ID), /* Optional */
	OBJ_FIELD_DATA(PORTFOLIO_AUTH_DATA_ID, R_OPT, OPAQUE), /* Optional Multi instance */
	OBJ_FIELD_DATA(PORTFOLIO_AUTH_STATUS_ID, R_OPT, U8), /* Optional: Single */
};
...
static struct lwm2m_engine_obj_inst *portfolio_create(uint16_t obj_inst_id)
{
	...
	INIT_OBJ_RES_MULTI_OPTDATA(PORTFOLIO_IDENTITY_ID, res[avail], i, res_inst[avail], j,
				   PORTFOLIO_IDENTITY_MAX, false);
...
static int lwm2m_portfolio_init(const struct device *dev)
{
	...
	portfolio.max_instance_count = ARRAY_SIZE(inst);
