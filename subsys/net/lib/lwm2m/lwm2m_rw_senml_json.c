			      ...
			      const char *format)
{
	...
	if (fd->add_base_name_to_start) {
		temp_length = snprintk(json_buffer, sizeof(json_buffer), "%s{\"bn\":\"/%u/%u/\",", sep,
				  path->obj_id, path->obj_inst_id);
	} else {
		temp_length = snprintk(json_buffer, sizeof(json_buffer), "%s{", sep);
	}
	...
	if (fd->writer_flags & WRITER_RESOURCE_INSTANCE) {
		temp_length = snprintk(json_buffer + len, sizeof(json_buffer) - len , "\"n\":\"%u/%u\",%s:",
			       path->res_id, path->res_inst_id, format);
	} else {
		temp_length = snprintk(json_buffer + len, sizeof(json_buffer) - len , "\"n\":\"%u\",%s:",
			       path->res_id, format);
	}
...
int do_composite_read_op_senml_json(struct lwm2m_message *msg)
{
	...
	path_list_size = json_parse_composite_read_paths(msg, &lwm2m_path_list, &lwm2m_path_free_list);
	if (path_list_size == 0) {
