...
struct json_out_formatter_data {
	...
	char bn_string[sizeof("/65535/999/") + 1];
	char name_string[sizeof("/65535/999/") + 1];
...
static const struct json_obj_descr senml_descr[] = {
	JSON_OBJ_DESCR_PRIM_NAMED(struct senml_struct, "bn",
				  base_name, JSON_TOK_STRING),
	JSON_OBJ_DESCR_PRIM_NAMED(struct senml_struct, "n",
				  name, JSON_TOK_STRING),
	JSON_OBJ_DESCR_PRIM_NAMED(struct senml_struct, "v",
				  val_float, JSON_TOK_FLOAT),
	JSON_OBJ_DESCR_PRIM_NAMED(struct senml_struct, "vb",
				  val_bool, JSON_TOK_TRUE),
	JSON_OBJ_DESCR_PRIM_NAMED(struct senml_struct, "vlo",
				  val_object_link, JSON_TOK_STRING),
	JSON_OBJ_DESCR_PRIM_NAMED(struct senml_struct, "vd",
				  val_opaque, JSON_TOK_OPAQUE),
	JSON_OBJ_DESCR_PRIM_NAMED(struct senml_struct, "vs",
...
struct json_in_formatter_data {
	...
	struct senml_struct senml_object;
};
...
static int json_float_object_write(struct lwm2m_output_context *out,
				   struct json_out_formatter_data *fd, int float_string_lenght)
{
	int res, len;
	ssize_t o_len;
	...
	if (fd->add_base_name_to_start) {
		...
		fd->json.obj.float_bn_obj.val_float.length = float_string_lenght;
		...
		fd->json.obj.float_obj.val_float.length = float_string_lenght;
	}
 ...
 */
static size_t base64_encoded_lenght(size_t slen)
{
	size_t n;
			 ...
			 size_t buflen)
{
	...
	encoded_lenght = base64_encoded_lenght(buflen);
	if (encoded_lenght < 0) {
		return -ENOMEM;
	}
	...
	if (fd->add_base_name_to_start) {
		...
		fd->json.obj.opaque_bn_obj.val_opaque.length = encoded_lenght;
		...
		fd->json.obj.opaque_obj.val_opaque.length = encoded_lenght;
	}
	...
	/* Put Original payload lenght back */
	if (fd->add_base_name_to_start) {
...
static int get_string(struct lwm2m_input_context *in, uint8_t *buf, size_t buflen)
{
	...
	size_t string_lengh = strlen(fd->senml_object.val_string);

	if (string_lengh > buflen) {
		LOG_WRN("Buffer too small to accommodate string, truncating");
		string_lengh = buflen - 1;
	}
	memcpy(buf, fd->senml_object.val_string, string_lengh);
